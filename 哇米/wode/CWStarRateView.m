...
{
    _backgroundImgName = bgImg;
    _foregroundImgName = foreImg;
    return [self initWithFrame:frame numberOfStars:numberOfStars];
}
...
- (void)buildDataAndUI {
    ...
    _isAllowTouch = YES; //默认可以触摸
    
    if(_foregroundImgName ==nil)
    {
        self.foregroundStarView = [self createStarViewWithImage:FOREGROUND_STAR_IMAGE_NAME];
    }
    else
    {
        self.foregroundStarView = [self createStarViewWithImage:_foregroundImgName];
    }
    
    if (_backgroundImgName==nil)
    {
        self.backgroundStarView = [self createStarViewWithImage:BACKGROUND_STAR_IMAGE_NAME];
    }
    else
    {
        self.backgroundStarView = [self createStarViewWithImage:_backgroundImgName];
    }
...
- (void)setForegroundImgName:(NSString *)foregroundImgName
{
//    [self buildDataAndUI];
    self.foregroundStarView = [self createStarViewWithImage:_foregroundImgName];
}
